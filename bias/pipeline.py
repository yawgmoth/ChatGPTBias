...
def main(directory, phases=["make", "get", "parse", "evaluate"], n=5, force=False, df=None, model="gpt-3.5-turbo-0125"):
    if "make" in phases:
        many_prompts.main(directory, n, df)
    if "get" in phases:
        get_response.main(directory, force, model)
    if "parse" in phases:
        parse_response.main(directory)
    if "evaluate" in phases:
        evaluate_results.main(directory)
...
if __name__ == "__main__":
    ...
    main(directory, phases, args.count, args.force, args.data, model)